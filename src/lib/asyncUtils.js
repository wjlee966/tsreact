...
export const createPromiseSaga = (type, promiseCreator) => {
  ...
  return function* saga(action) {
    try {
      // 재사용성을 위하여 prmoiseCreator의 파라미터엔 action.payload값을 넣도록 설정합니다.
...
export const reducerUtils = {
  ...
  // 바꿀 수도 있습니다.
  initial: (initialData = null) => ({ data: initialData, loading: false, error: null }),
  // 로딩 중 상태. prevState의 경우엔 기본값은 null이지만
  // 따로 값을 지정하면 null로 바꾸지 않고 다른 값을 유지시킬 수 있습니다.
  loading: (prevState = null) => ({ data: prevState, loading: true, error: null }),
  // 성공 상태
  success: payload => ({ data: payload, loading: false, error: null }),
  // 실패 상태
