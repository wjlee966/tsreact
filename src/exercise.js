...
/* 액션 생성 함수 정의 */
// 액션 생성 함수는 주로 camelCase 로 작성합니다.
// 화살표 함수로 작성하는 것이 더욱 코드가 간단하기에,
// 이렇게 쓰는 것을 추천합니다.
const increase = () => ({ type: INCREASE }); // 액션 객체에는 type 값이 필수입니다.
const decrease = () => ({ type: DECREASE });
const changeText = text => ({
  type: CHANGE_TEXT,
  text, // 액션 안에는 type 외에 추가적인 필드를 마음대로 넣을 수 있습니다.
});
...
console.log(store.getState()); // 현재 store 안에 있는 상태를 조회합니다.
...
// unSubscribe(); // 구독을 해지하고 싶을 때는 unSubscribe() 호출하면 됩니다.
